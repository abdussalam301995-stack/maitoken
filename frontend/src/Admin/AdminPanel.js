    ...
    function AdminPanel({ back }) {
      ...
      const renderTasksMissions = () => {
        ...
        const resetTaskForm = () => setTaskForm({title:'',targetUrl:'',reward:'',taskType:'telegram_join',telegramChatId:'',description:'',limitMode:'all',customLimit:'',refreshMode:'once',customHours:'',inviteCount:'1',holdingAmount:'0',miningClaims:'1',botEventType:''});
        ...
        const createTask = async (status='draft') => {
          ...
          try{
            ...
            flash(status==='active'?'Task published.':'Task draft created.'); resetTaskForm(); setTaskComposerOpen(false); await loadAll(true);
        ...
        return (
          <>
            ...
            {taskComposerOpen && <section className="adminSection taskComposer">
            ...
            {taskMode==='history'&&<div className="adminList">{[...managedTasks.filter(x=>x.status==='ended').map(x=>({...x,_kind:'task'})),...managedMissions.filter(x=>x.status==='ended').map(x=>({...x,_kind:'mission'}))].map((item,index)=><article className="adminListCard taskHistoryCard" key={`${item._kind}-${item.id}-${index}`}><div className="adminListTop"><div className="adminBadgeIcon">☷</div><div className="adminGrow"><b>{item.title}</b><span>Ended {item._kind} #{item.id} · evidence retained</span></div><Status>ended</Status></div><div className="adminActions"><button className="taskDeleteBtn" disabled={busy===`${item._kind}-delete-${item.id}`} onClick={()=>deleteModuleHistory(item._kind,item)}>{busy===`${item._kind}-delete-${item.id}`?'Deleting…':'⌫ Delete History'}</button></div></article>)}</div>}
          </>
        );
      };
